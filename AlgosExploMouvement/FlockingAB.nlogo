...
to setup
  ...
  crt population
    [
      ifelse random 20 = 0 [set color orange] [ifelse random 2 = 1 [set color blue] [set color green]]
...
to find-orange
  ifelse color != green [set oranges other turtles in-radius 0] 
  [set oranges other turtles in-radius vision with [color = orange]]
end

to find-flockmates  ;; turtle procedure
  ifelse color = orange
  [set flockmates other turtles in-radius 0]
...
to disperse 
  turn-away ([heading] of min-one-of oranges [distance myself]) 180
end
;;; ALIGN
