...
to flock  ;; turtle procedure
    find-flockmates
    if any? flockmates
    [let vectRep angle-obs
    let a angleFromVect additionvect vectDirect vectRep
...
to-report angle-obs
  ...
  if any? obstacles [
    set nearest-obstacles min-one-of obstacles [distance myself]
    let coeff dist-min / distance min-one-of obstacles [distance myself]
    let R VectFromAngle (towards nearest-obstacles + 180 ) (1 / distance nearest-obstacles)
    let vr multiplyScalarvect coeff R
    report vr
  ]
    set ennemis turtles in-cone vision angle-avoidance with [color != [color] of myself]
  if any? ennemis [
    set nearest-ennemy min-one-of ennemis [distance myself]
    let coeff dist-min / distance min-one-of ennemis [distance myself]
    let R VectFromAngle (towards nearest-ennemy + 180 ) (1 / distance nearest-ennemy)
    let vr multiplyScalarvect coeff R
    report vr
  ]
  report list 0 0
