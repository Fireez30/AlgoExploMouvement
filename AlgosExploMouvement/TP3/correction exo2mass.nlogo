...
to setup
  ...
  crt population
    [ ;;set color green - 2 + random 7  ;; random shades look nice
      ...
      set acceleration 0
      set mass random 5 + 0.2
      set size 1.5 ; 4 / (mass + 1)  ;; easier to see
...
to flock  ;; turtle procedure
  ...
  [let  v  vectDirect
    ...
    let f 0
    set acceleration (s * amount-acceleration) / mass
    if friction? [set f friction]
    set acceleration  acceleration - f
  ]
  [ let v vectWithObstacles
    ...
    turn-towards a max-angle-turn
    let f 0
    set acceleration (base-speed * amount-acceleration) / mass
    if friction? [set f friction]
    set acceleration  acceleration - f
