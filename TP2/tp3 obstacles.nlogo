...
to assign-goal 
    if (but = 0) [set but max-one-of patches with [goal = 1] [distance myself]]
end
...
to compute-repulsive1
  ask patches [
    if (goal = 0) [
   let r-old repulsive
          let case2 min-one-of neighbors [repulsive]
          let r ([repulsive] of case2) + 1

          if (repulsive >= r and r != r-old) [
            set repulsive r
            ifelse repulsive > 0
              [set repulsive 0]
              [set continue true]]
...
to compute-potential1
   ask patches [
        if (goal = 0 and obstacle = 0) [
          ...
          if (potential <= p  and p != v-old) [
            set potential p
          ifelse potential < 0
            [set potential 0]
            [if potential + repulsive >= 0
               [set continue true]]]
...
to eviter-obstacles
  let p min-one-of patches in-cone 4 55 with [obstacle = 1] [distance myself]
  if p != nobody [set heading towards p
    rt 90 ]
end
...
to go-goal
  ...
  ifelse but = 0 [
    assign-goal ]
   [    
     ifelse cooldown = 0 [set heading towards but] []
     ifelse (patch-ahead 1 = nobody) [rt 180] [ifelse ([obstacle] of patch-ahead 1 != 1) [advance] [manoeuvre-evitement]]
  ]
end

to manoeuvre-evitement
   if cooldown = 0 [set cooldown cooldown-max]
  ifelse ([obstacle] of patch-ahead 1 != 1) [advance] [set heading random 360 set cooldown cooldown + ajout-colere]
end
to advance
  rt random 15
  lt random 15 
